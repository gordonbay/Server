            ...
            if(cid) then
                ...
                if(full_weight <= free_cap) then
                    ...
                    if(type(received_item) == "number" and received_item == RETURNVALUE_NOERROR) then
                        doPlayerSendTextMessage(cid, SHOP_MSG_TYPE, 'Tu orden del item/s '.. add_item_name ..' del Web-Shop. Ya fue procesada ya deberias tener tu item en la backpack.')
						setPlayerStorageValue(cid,storage+555884621212,1)
                        db.query("DELETE FROM `z_ots_comunication` WHERE `id` = " .. id .. ";")
                        db.query("UPDATE `z_shop_history_item` SET `trans_state`='realized', `trans_real`=" .. os.time() .. " WHERE id = " .. id .. ";")
                    else
                        doPlayerSendTextMessage(cid, SHOP_MSG_TYPE, ' La orden del item/s  '.. add_item_name ..' del Web-Shop esta en espera. Por favor haz espacio en tu backpack/mano y espera como '.. SQL_interval ..' para obtenerlo.')
                    end
                else
                    doPlayerSendTextMessage(cid, SHOP_MSG_TYPE, 'La orden del item/s '.. add_item_name ..'  del Web-Shop esta en espera. El item pesa '.. full_weight ..' oz., y tu nada mas tienes '.. free_cap ..' oz. disponibles de capacidad. Pon algunos items en el Depot y espera como '.. SQL_interval ..' segundos para recibir la orden.')
                end
            end
